        ...
        if ($_SERVER["REQUEST_METHOD"] == "GET" && !empty($_GET["proy"])) {
          ...
          if (isset($_SESSION["userID"]) && 
            ...
            if ($result->num_rows == 1) {
              ...
              echo '<div class="col"><form><div class="form-row">
                      <div class="form-group col">
                          <input type="number" id="quantity" min="0" max="10" class="form-control" name="quantity" placeholder="Calificacion: 0 - 10">
                      </div>
                      <div class="form-group col">
                        <button type="submit" name="fav" value="'.$IDProy.'" class="btn btn-primary btn-info btn-block">Calificar</button>
                      </div></div></form></div>';
              echo '</div></div></div>';
              // if ($optionsComp = $conn->prepare("SELECT ID_Componente, Nombre
              //                                   FROM componentes;")) {
